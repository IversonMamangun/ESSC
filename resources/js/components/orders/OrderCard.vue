...
import { Truck, Star, RotateCcw, MessageSquare } from 'lucide-vue-next'; // Import relevant icons
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
// Define emits to pass events back up to Index.vue
defineEmits<{
...
<template>
  <Card>
          ...
          <AccordionContent>
            <div class="space-y-4">
              <div v-for="(item, index) in order.items" :key="index" class="flex gap-4">
                <Link :href="shop.orders.show.url({ order: order.id })" class="shrink-0 hover:opacity-80 transition-opacity">
                  <img :src="item.product_image ?? '/images/placeholder-product.png'" :alt="item.product_name" class="h-20 w-20 rounded-lg border object-cover" />
                </Link>
                <div class="flex flex-1 justify-between">
                  <div>
                    <Link :href="shop.orders.show.url({ order: order.id })" class="font-medium hover:text-[#009933] transition-colors line-clamp-1 hover:underline">
                      {{ item.product_name }}
                    </Link>
                    <p v-if="item.variant_name" class="text-sm text-muted-foreground">{{ item.variant_name }}</p>
    ...
    <CardFooter class="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-4 border-t pt-4">
      <div class="flex flex-wrap gap-2 w-full sm:w-auto">
        
        <template v-if="order.status === 'to-receive'">
          <Link
            :href="shop.orders.show.url({ order: order.id })"
            class="inline-flex items-center justify-center rounded-xl bg-zinc-900 px-4 py-2 text-sm font-bold text-white hover:bg-zinc-800 transition-colors dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-200 cursor-pointer"
          >
        ...
        <template v-if="order.status === 'completed'">
          <button
            ...
            class="inline-flex items-center justify-center rounded-xl bg-[#009933] px-4 py-2 text-sm font-bold text-white hover:bg-[#007722] transition-colors cursor-pointer"
          >
      ...
      <div class="flex flex-col items-end gap-0.5 w-full sm:w-auto border-b pb-3 sm:border-0 sm:pb-0">
        <div class="text-xs text-muted-foreground">
          Shipping: {{ formatPrice(order.shipping_fee) }}
        </div>
        <div class="text-lg font-black text-zinc-900 dark:text-white">
          Total: <span class="text-[#009933]">{{ formatPrice(order.total) }}</span>
        </div>
      </div>
    </CardFooter>
  </Card>
