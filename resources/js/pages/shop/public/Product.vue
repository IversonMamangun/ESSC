...
// Use passed product or fallback to sample data
const productData = computed(() => props.product || sampleProduct);
...
const isComingSoon = computed(() => parseFloat(productData.value?.price) <= 0);
...
const isDiscounted = computed(() => {
  ...
  const discount = parseFloat(productData.value?.discount_price);
...
const increaseQuantity = () => {
  if (quantity.value < (productData.value.stock || 100)) {
    quantity.value++;
  }
};
...
onMounted(() => {
  ...
  console.log('Product data:', productData.value); // Debug log
});
</script>

<template>
  <Head :title="`${productData.title} - Store`" />
  ...
  <div class="flex min-h-screen flex-col transition-colors duration-300">
    ...
    >
      ...
      <div class="mb-4 rounded-lg bg-yellow-100 p-2 text-sm text-yellow-800">
        Debug: Product ID: {{ productData.id }} | Title: {{ productData.title }}
      </div>
      ...
      >
        ...
        <Link href="/store" class="transition-colors hover:text-[#009933]"
          >Online Store</Link
        >
        <ChevronRight class="h-4 w-4" />
        <span
